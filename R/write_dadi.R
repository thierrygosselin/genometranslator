...
write_dadi <- function(
  ...
  # create a strata.df
  strata.df <- genometranslator::generate_strata(data = data, pop.id = TRUE)
  ...
  if (!tibble::has_name(data, "GT_VCF") && !tibble::has_name(data, "ALT_DOSAGE")) {
  ...
  if (calibrate.alleles) {
    data %<>% calibrate_alleles(data = ., alt.dosage = TRUE) %$% input
  }
  ...
  if (tibble::has_name(data, "ALT_DOSAGE")) {
    input.count <- data %>%
      ...
      dplyr::summarise(
        N = as.numeric(n()),
        PP = as.numeric(length(ALT_DOSAGE[ALT_DOSAGE == 0L])),
        PQ = as.numeric(length(ALT_DOSAGE[ALT_DOSAGE == 1L])),
        QQ = as.numeric(length(ALT_DOSAGE[ALT_DOSAGE == 2L])),
        .groups = "keep"
      ) %>%
  ...
  if (is.null(fasta.ingroup)) {
    dadi.input <- suppressWarnings(
      input.count %>%
        ...
        tgbase::trans_wide(x = ., formula = "MARKERS + Allele1 + Allele2 ~ POP", values_from = "COUNT") %>%
        dplyr::mutate(
