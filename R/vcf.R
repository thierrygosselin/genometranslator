...
#'
#' VCF import uses the declared Bioconductor packages \pkg{SeqArray},
#' \pkg{gdsfmt}, and \pkg{Rsamtools}. VCF preparation and indexing may also
#' require the optional \command{bcftools} executable. See the installation
#' section of the package README and check visibility with
#' \code{Sys.which("bcftools")}.
#'
#' @name read_vcf
#' @rdname read_vcf
#' @export
...
#' \code{/metadata} node containing genome-specific metadata.
#'
...
#'   \item a 21 GB VCF with ~2M SNPs: \eqn{\sim}4 minutes with 8 CPU.
#' }
#'
...
#'
#' @return
#' A \code{SeqVarGDSClass} object (GDS) with a \code{/metadata} node
...
read_vcf <- function(
  ...
  individuals.vcf <- tibble::tibble(
    ...
    dplyr::mutate(
      INDIVIDUALS_CLEAN = clean_ind_names(INDIVIDUALS_VCF)
    )
...
if (verbose) message("VCF-specific filters")
  ...
  path.folder.filtered <- tgbase::generate_folder(
    folder  = "filtered",
    path.folder = wf,
    internal    = FALSE,
    file.date   = file.date,
...
#'
#' It is highly recommended to use \code{\href{https://thierrygosselin.github.io/radr/reference/explore_genomes.html}{radr::explore_genomes}} to reduce
...
#' Documented in \code{\link{tidy_genome}}.
...
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_whitelist.html}{radr::filter_whitelist}}.
#' \item \code{filter.individuals.missing}: (double)
#' Default: \code{filter.individuals.missing = NULL}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_individuals.html}{radr::filter_individuals}}.
#' \item \code{filter.monomorphic}: (logical)
#' Default: \code{filter.monomorphic = TRUE}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_monomorphic.html}{radr::filter_monomorphic}}.
#' Required package: \code{UpSetR}.
#' \item \code{filter.common.markers}: (logical)
#' Default: \code{filter.common.markers = TRUE}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_common_markers.html}{radr::filter_common_markers}}.
#' Required package: \code{UpSetR}.
#' \item \code{filter.ma}: (integer)
#' Default: \code{filter.ma = NULL}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_ma.html}{radr::filter_ma}}.
#' \item \code{filter.coverage}: (logical)
#' Default: \code{filter.coverage = NULL}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_coverage.html}{radr::filter_coverage}}.
#' \item \code{filter.genotyping}: (integer)
#' Default: \code{filter.genotyping = NULL}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_genotyping.html}{radr::filter_genotyping}}.
#' \item \code{filter.snp.position.read: } (optional, character, integer)
#' Default: \code{filter.snp.position.read = NULL}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_snp_position_read.html}{radr::filter_snp_position_read}}.
#' \item \code{filter.snp.number: } (optional, character, integer)
#' Default: \code{filter.snp.number = NULL}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_snp_number.html}{radr::filter_snp_number}}.
#' \item \code{filter.short.ld}: (optional, character)
#' Default: \code{filter.short.ld = NULL}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_ld.html}{radr::filter_ld}}.
#' \item \code{filter.long.ld}: (optional, character)
#' Default: \code{filter.long.ld = NULL}.
#' Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_ld.html}{radr::filter_ld}}.
#' Required package: \code{SNPRelate}.
#' \item \code{long.ld.missing}: Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_ld.html}{radr::filter_ld}}.
#' Default: \code{long.ld.missing = FALSE}.
#' \item \code{ld.method}: Documented in \code{\href{https://thierrygosselin.github.io/radr/reference/filter_ld.html}{radr::filter_ld}}.
...
#' \code{\link{tidy_genome}},
#' \code{\href{https://thierrygosselin.github.io/radr/reference/explore_genomes.html}{radr::explore_genomes}}
...
tidy_vcf <- function(
  ...
  on.exit(tgbase::function_header(f.name = "tidy_vcf", start = FALSE, verbose = verbose), add = TRUE)
  ...
  # import VCF -----------------------------------------------------------------
  data <- genometranslator::read_vcf(
    data = data,
    strata = strata,
    filename = filename,
    verbose = FALSE,
  ...
  # tidy_vcf folder ------------------------------------------------------------
  tidy.folder <- tgbase::generate_folder(
    folder = "tidy_vcf",
    path.folder = path.folder,
    prefix.int = TRUE,
    internal = FALSE,
  ...
  tgbase::write_tgbase_tsv(
    data = rad.dots,
    path.folder = path.folder,
    filename = "genometranslator_tidy_vcf_args",
    date = TRUE,
    internal = internal,
    write.message = "Function call and arguments stored in: ",
    verbose = verbose
  ...
  message("    ALT_DOSAGE (the dosage of ALT allele: 0, 1, 2 NA): ", alt.dosage)
  ...
  if (vcf.metadata) {
    ...
    if (rlang::has_name(tidy.data, "C_DEPTH")) {
      ...
      #   ) %>%
      #   tgbase::trans_long(cols = "M_SEQ", names_to = "ACGT_DEPTH", values_to = "DEPTH", tidy = TRUE) %>%
      ...
      #   ) %>%
      #   dplyr::group_by(M_SEQ) %>%
      #   tgbase::trans_wide(names_from = c("ALLELE", "ALLELE_DEPTH"), values_from = c("NUCLEOTIDE", "DEPTH"), tidy = TRUE) %>%
    ...
    if (all(rlang::has_name(tidy.data, c("ALLELE_REF_DEPTH", "ALLELE_ALT_DEPTH", "READ_DEPTH")))) {
      ...
      if (stacks.prob > 0) {
        non.missing.gt <- length(tidy.data$ALT_DOSAGE[!is.na(tidy.data$ALT_DOSAGE)])
        ...
        stacks.ad <- dplyr::select(tidy.data, ALT_DOSAGE) %>%
          dplyr::bind_cols(dplyr::select(tidy.data, READ_DEPTH, ALLELE_REF_DEPTH, ALLELE_ALT_DEPTH)) %>%
          dplyr::mutate(COL_ID = seq(1, n())) %>%
          dplyr::filter(!is.na(READ_DEPTH)) %>%
          dplyr::filter(is.na(ALLELE_REF_DEPTH) & is.na(ALLELE_ALT_DEPTH)) %>%
          dplyr::mutate(
            ALLELE_REF_DEPTH = dplyr::if_else(ALT_DOSAGE == 0, READ_DEPTH, ALLELE_REF_DEPTH),
            ALLELE_ALT_DEPTH = dplyr::if_else(ALT_DOSAGE == 2, READ_DEPTH, ALLELE_ALT_DEPTH)
          )
  ...
  if (calibrate.alleles) {
    tidy.data %<>% calibrate_alleles(data = ., biallelic = biallelic, verbose = verbose) %$% input
  }

  # Join back the info ---------------------------------------------------------
  tidy.data %<>%
    join_genome_metadata(
      x = .,
  ...
  write_genome(data = tidy.data, filename = filename.rad$filename, verbose = verbose)

  if (verbose) message("Updating GDS with genotypes.meta values")
  update_genome_gds(gds = data, node.name = "genotypes.meta", value = tidy.data)
...
#' @keywords internal
#' @param exclude
#' Default: \code{exclude = NULL}.
#'
#' @param extra
#' Default: \code{extra = NULL}.
#'
#' @export
...
#' @author Thierry Gosselin \email{thierrygosselin@@icloud.com}
#' @param safety.margin
#' Default: \code{safety.margin = 0.9}.
#'
#' @param allowed
#' Default: \code{allowed = c(1, 0.3, 0.2, 0.1, 0.05, 0.02, 0.01)}.
#'
#' @param cap.prop
#' Default: \code{cap.prop = 0.3}.
#'
#' @param small.vcf.cutoff
#' Default: \code{small.vcf.cutoff = 2e+05}.
#'
#' @param n.matrices
#' Default: \code{n.matrices = 1}.
#'
#' @param bytes.per.cell
#' Default: \code{bytes.per.cell = 4}.
#'
#' @param max.mem.gb
#' Default: \code{max.mem.gb = 10}.
#'
#' @param vector.size.limit
#' Default: \code{vector.size.limit = 2^31}.
#'
#' @export
...
write_vcf <- function(
  ...
  if (empty) {
    ...
    if (!tibble::has_name(data, "GT_VCF")) {
      data %<>% calibrate_alleles(data = ., gt.vcf = TRUE, gt.vcf.nuc = TRUE) %$% input
    }
    ...
    if (strata) {
      ...
      output <- suppressWarnings(
        dplyr::left_join(data, info.field, by = "MARKERS") %>%
          ...
          tgbase::trans_wide(x = ., formula = "MARKERS + CHROM + LOCUS + POS + INFO + REF + ALT ~ INDIVIDUALS", values_from = "GT_VCF") %>%
          dplyr::mutate(
            QUAL = rep(".", n()),
            FILTER = rep("PASS", n()),
            FORMAT = rep("GT", n())
...
#' @seealso \pkg{genometranslator} \code{\link{read_strata}}
#' @author Thierry Gosselin \email{thierrygosselin@icloud.com}
#'
#' @examples
#' \dontrun{
...
#'
#' Default: \code{markers.info = NULL}.
...
#' @name clean_vcf_markers_meta
#' @rdname clean_vcf_markers_meta
#' @title title Clean and normalise markers metadata from a VCF import
...
# }
# It is designed for internal use in \pkg{genometranslator} and is not exported.
...
#' Default: \code{stacks.checks = FALSE}.
...
#' }
#' @name filter_vcf_filter_column
#' @rdname filter_vcf_filter_column
#' @keywords internal
#' @export
...
#' Default: \code{detect.strand = FALSE}.
...
#' @param internal (logical) Passed to the internal results-message helper and
#'   \code{genome_parameters()}.
...
filter_duplicated_markers_strands <- function(
  ...
  path.folder.strands <- tgbase::generate_folder(
    folder  = "filter_duplicated_markers",
    path.folder = path.folder,
    internal    = FALSE,
    file.date   = file.date,
    verbose     = verbose
...
#' The current filter-parameter object generated by \code{genome_parameters()}.
#'
...
filter_haplotype_format <- function(
  ...
  if (!isTRUE(filter.haplotype.format)) {
    markers.meta <- genometranslator::extract_markers_metadata(gds = gds)
    return(list(
      markers.meta       = markers.meta,
      filters.parameters = filters.parameters
    ))
  ...
  blacklist.multiallelic <- genometranslator::extract_markers_metadata(
    gds       = gds,
    whitelist = TRUE
  ) %>%
    dplyr::mutate(
  ...
  if (n.snp.m > 0L) {
    ...
    # Folders ------------------------------------------------------------------
    path.folder.biallelic <- tgbase::generate_folder(
      folder  = "filter_haplotype_format",
      path.folder = path.folder,
      internal    = FALSE,
      file.date   = file.date,
      verbose     = verbose
...
#'
#' Default: \code{fail_on_status = TRUE}.
#' @return
