...
#' @title Genome GDS constructor
#' @description helper function to construct a genome GDS
#' @name genome_gds
#' @rdname genome_gds
#' @keywords internal
#' @param verbose 
...
#' 
#' @param open 
#' Default: \code{open = FALSE}.
#' 
#' @param filename 
#' Default: \code{filename = NULL}.
#' 
#' @param gl 
#' Default: \code{gl = NULL}.
#' 
#' @param ad 
#' Default: \code{ad = NULL}.
#' 
#' @param dp 
#' Default: \code{dp = NULL}.
#' 
#' @param genotypes.meta 
#' Default: \code{genotypes.meta = NULL}.
#' 
#' @param markers.meta 
#' Default: \code{markers.meta = NULL}.
#' 
#' @param strata 
...
genome_gds <- function(
  ...
  # genome metadata skeleton folder ---------------------------------------------------
  metadata.node <- genome_gds_skeleton(data.gds)

  # data.source ----------------------------------------------------------------
  if (is.null(data.source)) data.source <- "genometranslator"
  update_genome_gds(data.gds, node.name = "data.source", value = data.source)

  # bi- or multi-alllelic VCF --------------------------------------------------
  update_genome_gds(data.gds, node.name = "biallelic", value = biallelic)

  # Coverage -------------------------------------------------------------------
  update_genome_gds(data.gds, node.name = "DP", value = dp)
  update_genome_gds(data.gds, node.name = "AD", value = ad)
  update_genome_gds(data.gds, node.name = "GL", value = gl)
  ...
  update_genome_gds(data.gds, node.name = "markers.meta", value = markers.meta)
  ...
  update_genome_gds(gds = data.gds, node.name = "individuals.meta", value = strata)
  update_genome_gds(
    gds = data.gds,
    node.name = "sample.id",
...
# gt2array----------------------------------------------------------------------
#' @title gt2array
#' @description Alternate allele dosage gt (ALT_DOSAGE) to
#' presence/absence array for the genotypes in GDS
#' @rdname gt2array
#' @keywords internal
#' @param n.snp 
#' Default: \code{n.snp = NULL}.
#' 
#' @param n.ind 
#' Default: \code{n.ind = NULL}.
#' 
#' @export
...
# tidy2gds ---------------------------------------------------------------------
#' @title tidy2gds
#' @description Generate a GDS file/object from a tidy dataset requires (ALT_DOSAGE)
#' @rdname tidy2gds
#' @keywords internal
#' @export
tidy2gds <- function(x) {
...
#'
#' @rdname tidy_genome
#' @param data A GDS filename or open \code{SeqVarGDSClass} object.
#' @param pop.id Logical. When \code{pop.id = TRUE}, the strata column is
#' named \code{POP_ID}; otherwise it is named \code{STRATA}.
#' Default: \code{pop.id = FALSE}.
#' @param close.gds Logical. Close an open GDS object supplied by the caller.
#' A GDS opened internally from a filename is always closed.
#' Default: \code{close.gds = FALSE}.
#' 
#' @param parallel.core 
#' Default: \code{parallel.core = parallel::detectCores() - 1}.
#' 
#' @param strip.rad 
#' Default: \code{strip.rad = FALSE}.
#' 
#' @param calibrate.alleles 
#' Default: \code{calibrate.alleles = TRUE}.
#' 
#' @param individuals 
#' Default: \code{individuals = NULL}.
#' 
#' @param wide 
#' Default: \code{wide = FALSE}.
#' 
#' @param markers.meta.select 
#' Default: \code{markers.meta.select = NULL}.
#' 
#' @param markers.meta 
#' Default: \code{markers.meta = NULL}.
#' 
#' @return A tidy genomic data frame.
#' @export
tidy_genome <- function(
    data,
    markers.meta = NULL,
  ...
  gds <- data
  opened.internally <- FALSE

  # test
  # gt <- gt.vcf.nuc <- gt.vcf <- alt.dosage <- FALSE
  ...
  alt.dosage    <- rad.dots[["alt.dosage"]]     %||% FALSE
  ...
  if (data.type == "gds.file") {
    gds %<>% genometranslator::read_genome(data = ., verbose = TRUE, allow.dup = TRUE)
    opened.internally <- TRUE
  }
  ...
  # Column names and marker identifier. Newer GDS metadata uses MARKERS,
  # whereas legacy radiator metadata can contain M_SEQ.
  if (rlang::has_name(markers.meta, "M_SEQ")) {
    cn <- dplyr::pull(markers.meta, M_SEQ)
    want.m <- "M_SEQ"
  } else {
  ...
  if (calibrate.alleles && !wide) {
    tidy.data %<>%
      calibrate_alleles(
        ...
        gt.vcf = gt.vcf,
        alt.dosage = alt.dosage,
        verbose = FALSE
      ) %$% input
  }

  #  close the connection with GDS file
  if (close.gds || opened.internally) {
    SeqArray::seqClose(gds)
    gds <- NULL
  }

  if (rlang::has_name(tidy.data, "VARIANT_ID")) {
    tidy.data %<>% dplyr::arrange(VARIANT_ID, .data[[want.id]])
  } else if (rlang::has_name(tidy.data, "MARKERS")) {
    tidy.data %<>% dplyr::arrange(MARKERS, .data[[want.id]])
  } else {
    tidy.data %<>% dplyr::arrange(.data[[want.id]])
  }
...
# Resolve package metadata paths while supporting legacy radiator GDS files.
genome_metadata_path <- function(gds, child = NULL) {
  current.node <- gdsfmt::index.gdsn(
    node = gds,
    path = "genometranslator",
    silent = TRUE
  )
  legacy.node <- gdsfmt::index.gdsn(
    node = gds,
    path = "radiator",
    silent = TRUE
  )

  root <- if (!is.null(current.node)) {
    "genometranslator"
...
#'   of the output path.
#' Default: \code{open = FALSE}.
#' @param verbose Whether to report migration progress.
#'
#' Default: \code{verbose = TRUE}.
...
upgrade_genome_gds <- function(
  ...
  if (source.connection) {
    source.filename <- gds$filename
    if (is.null(source.filename)) {
      rlang::abort("Cannot determine the filename of the open GDS connection.")
    }
    gdsfmt::sync.gds(gds)
  } else {
    if (length(gds) != 1L || is.na(gds) || !nzchar(gds)) {
      rlang::abort("`gds` must be one GDS filename or an open GDS connection.")
    }
    source.filename <- gds
  }
  ...
  output.dir <- dirname(filename)
  if (!dir.exists(output.dir)) {
    rlang::abort(paste0("Output directory does not exist: ", output.dir))
  }

  source.key <- normalizePath(source.filename, mustWork = TRUE)
  output.key <- file.path(normalizePath(output.dir, mustWork = TRUE), basename(filename))
  if (identical(source.key, output.key)) {
    rlang::abort("The output filename must differ from the source filename.")
  }
  ...
  temporary.file <- tempfile(
    pattern = ".genometranslator-upgrade-",
    tmpdir = output.dir,
    fileext = ".gds"
  )
  on.exit(if (file.exists(temporary.file)) file.remove(temporary.file), add = TRUE)
  ...
  if (!isTRUE(copied)) {
    rlang::abort("Unable to create the temporary GDS copy.")
  }

  output.gds <- gdsfmt::openfn.gds(
  ...
  on.exit({
    if (isTRUE(connection.open)) {
      try(gdsfmt::closefn.gds(output.gds), silent = TRUE)
    }
  }, add = TRUE)
  ...
  gdsfmt::put.attr.gdsn(
    current.node,
    name = "schema",
    val = "genometranslator-gds"
  )
  gdsfmt::put.attr.gdsn(current.node, name = "schema_version", val = "1.0.0")
  gdsfmt::sync.gds(output.gds)
  ...
  if (!file.rename(temporary.file, filename)) {
    rlang::abort(paste0("Unable to finalize output file: ", filename))
  }
...
# genome_gds_skeleton---------------------------------------------------------
#' @title genome_gds_skeleton
#' @description Generate a genometranslator metadata skeleton inside GDS
#' @name genome_gds_skeleton
#' @rdname genome_gds_skeleton
#' @keywords internal
#' @export
genome_gds_skeleton <- function(gds) {
  metadata.node <- gdsfmt::addfolder.gdsn(
    node = gds,
    name = "genometranslator",
    replace = TRUE
  )
  gdsfmt::put.attr.gdsn(
    metadata.node,
  ...
  metadata.node
} # End genome_gds_skeleton

# update_genome_gds-----------------------------------------------------------
#' @title update_genome_gds
#' @description Update a genome GDS metadata node
#' @name update_genome_gds
#' @rdname update_genome_gds
#' @keywords internal
#' @param verbose 
#' Default: \code{verbose = FALSE}.
#' 
#' @param summary 
#' Default: \code{summary = FALSE}.
#' 
#' @param sync 
#' Default: \code{sync = FALSE}.
#' 
#' @param replace 
#' Default: \code{replace = TRUE}.
#' 
#' @param metadata.node 
#' Default: \code{metadata.node = TRUE}.
#' 
#' @export
update_genome_gds <- function(
    gds,
    metadata.node = TRUE,
    node.name,
    value,
  ...
  if (node.name %in% c("AD", "DP", "GL")) {
    ...
    if (metadata.node) {
      metadata.node <- gdsfmt::index.gdsn(
        node = gds,
        path = genome_metadata_path(gds),
        silent = TRUE
      )
...
}#End update_genome_gds

# extract_data_source----------------------------------------------------------------
#' @title extract_data_source
#' @description Extract the data source of a genome GDS file
#' @name extract_data_source
#' @rdname extract_data_source
#' @keywords internal
#' @export
extract_data_source <- function(gds) {
  data.source <- gdsfmt::index.gdsn(
    node = gds,
    path = genome_metadata_path(gds, "data.source"),
    silent = TRUE
  )
...
#' @description Import individuals metadata from a package or SeqArray node
...
#' Default: \code{ind.field.select = NULL}.
#' @param metadata.node (optional, logical) Default:\code{metadata.node = TRUE}.
#' Default: \code{metadata.node = TRUE}.
...
extract_individuals_metadata <- function(
  ...
  if (snprelate) {
    ...
    if (metadata.node) {
      id.index <- gdsfmt::index.gdsn(
        node = gds,
        path = genome_metadata_path(gds, "individuals.meta"),
        silent = TRUE
      )
      if (is.null(id.index)) {
        metadata.node <- FALSE
    ...
    # Another metadata.node is necessary here don't remove it to include with previous...
    if (metadata.node) {
      ...
      id_fields <- function(x, gds, id.df) {
        id.df[[x]] <- gdsfmt::read.gdsn(
          gdsfmt::index.gdsn(
            node = gds,
            path = paste0(genome_metadata_path(gds, "individuals.meta"), "/", x),
            silent = TRUE))
      }
...
#' @description Import markers metadata from a genometranslator or SeqArray GDS.
#' @name extract_markers_metadata
#' @rdname extract_markers_metadata
#' @param gds The GDS object (genometranslator, legacy radiator, or SeqArray).
#' @param markers.meta.select (optional, character) Names of metadata fields to
#'   import. For package GDS files, these are the column names in the metadata
#'   node. For plain SeqArray GDS, standardized names
...
#'   Default: \code{markers.meta.select = NULL}.
#' @param metadata.node (logical) Whether to prefer the package metadata node
#'   if present. Both \code{genometranslator} and legacy \code{radiator} nodes
#'   are supported. If the node is missing or empty, the function falls back
#'   to SeqArray nodes. Default: \code{metadata.node = TRUE}.
...
extract_markers_metadata <- function(
  ...
  # Detect whether package markers metadata exists -----------------------------
  markers.index <- NULL

  if (metadata.node) {
    markers.index <- gdsfmt::index.gdsn(
      node  = gds,
      path  = genome_metadata_path(gds, "markers.meta"),
      silent = TRUE
    )
    ...
    if (is.null(markers.index) ||
      ...
      # No usable package markers metadata -> fall back to SeqArray
      metadata.node <- FALSE
      markers.index <- NULL
    } else {
  ...
  # For plain SeqArray GDS: map standardized names to SeqArray varnames --------
  if (!metadata.node) {
    if (!is.null(markers.meta.select)) {
      markers.meta.select %<>%
  ...
  integrate_meta <- function(x, gds, metadata.node) {
    if (metadata.node) {
      gdsfmt::read.gdsn(
        gdsfmt::index.gdsn(
          node   = gds,
          path   = paste0(genome_metadata_path(gds, "markers.meta"), "/", x),
          silent = TRUE
        )
      )
    } else {
...
#' Default: \code{genotypes.meta.select = NULL}.
...
extract_genotypes_metadata <- function(
  ...
  geno.index <- gdsfmt::ls.gdsn(gdsfmt::index.gdsn(
    node = gds,
    path = genome_metadata_path(gds, "genotypes.meta"),
    silent = TRUE
  ))
  ...
  if (!index.only) {
    ...
    integrate_meta <- function(x, gds, genotypes.df) {
      genotypes.df[[x]] <- gdsfmt::read.gdsn(
        gdsfmt::index.gdsn(
          node = gds,
          path = paste0(genome_metadata_path(gds, "genotypes.meta"), "/", x),
          silent = TRUE))
    }
...
#' Default: \code{allele.coverage = TRUE}.
...
extract_coverage <- function(
  ...
  coverage_stats <- function(
    ...
    data.source <- genometranslator::extract_data_source(gds = gds)
    ...
    if (dp) {
      ...
      if ("dart" %in% data.source) {
        dart.data <- genometranslator::extract_genotypes_metadata(
          gds = gds,
  ...
  cov_tibble_stats <- function(have, tibble.group, data, subsample.info) {
    if (have %in% names(data)) {
      cov.tib <- tgbase::tibble_stats(x = as.numeric(data[[have]]), group = tibble.group, subsample = subsample.info)
    } else {
      cov.tib <- NULL
    }
    return(cov.tib)
...
parse_gds_metadata <- function(
  ...
  m.temp <- extract_markers_metadata(
    gds = gds,
  ...
  if (length(m.temp) != n.markers) {
    v.select <- extract_markers_metadata(gds = gds, metadata.node = FALSE) %>% dplyr::pull("VARIANT_ID")
    m.temp <- extract_markers_metadata(gds = gds, metadata.node = TRUE, markers.meta.select = c("VARIANT_ID","M_SEQ"), whitelist = TRUE) %>%
    ...
    if ("AD" %in% format.name) {
      ...
      safe.ad <- safe_make_tibble(
        ALLELE_REF_DEPTH = temp.ad[, column.vec %% 2 == 1] %>%
          as.vector(.) %>% tgbase::replace_by_na(data = ., what = 0L),
...
sync_gds <- function(
  ...
  if (reset.gds || reset.filters.m || reset.filters.i) {
    ...
    if (reset.filters.m) {
      ...
      update_genome_gds(gds = gds, node.name = "markers.meta", value = m, sync = FALSE)
    ...
    if (reset.filters.i) {
      i <- extract_individuals_metadata(gds = gds, whitelist = FALSE) %>%
        dplyr::mutate(FILTERS = "whitelist")
      update_genome_gds(gds = gds, node.name = "individuals.meta", value = i, sync = FALSE)
      SeqArray::seqSetFilter(
...
reset_filters <- function(
  ...
  if (reset.all) {
    ...
    markers.meta <- extract_markers_metadata(gds = gds, whitelist = FALSE) %>%
      dplyr::mutate(FILTERS = "whitelist")
    # update
    update_genome_gds(gds = gds, node.name = "markers.meta", value = markers.meta, sync = FALSE)
    #reset
    ...
    individuals <- extract_individuals_metadata(gds = gds, whitelist = FALSE) %>%
      dplyr::mutate(FILTERS = "whitelist")
    # update
    update_genome_gds(gds = gds, node.name = "individuals.meta", value = individuals, sync = FALSE)
    #reset
...
#' @keywords internal
#' @param bl.gds 
#' Default: \code{bl.gds = NULL}.
#' 
#' @param extract 
#' Default: \code{extract = FALSE}.
#' 
#' @param update 
#' Default: \code{update = NULL}.
#' 
#' @param generate 
...
update_bl_markers <- function(
  ...
  if (is.null(metadata.node)) metadata.node <- gds

  #Run
  if (!generate) {
    test <- gdsfmt::get.attr.gdsn(gdsfmt::index.gdsn(
      node = metadata.node, path = "blacklist.markers", silent = TRUE))$R.class[1]
  ...
  if (extract) {
    bl.gds <- tibble::tibble(
      MARKERS = gdsfmt::read.gdsn(gdsfmt::index.gdsn(
        node = metadata.node, path = "blacklist.markers/MARKERS", silent = TRUE)),
      FILTER = gdsfmt::read.gdsn(gdsfmt::index.gdsn(
        node = metadata.node, path = "blacklist.markers/FILTER", silent = TRUE))
    )
  }
...
update_bl_individuals <- function(
  ...
  if (is.null(metadata.node)) metadata.node <- genome_gds_skeleton(gds)

  #Run
  if (!generate) {
    test <- gdsfmt::get.attr.gdsn(gdsfmt::index.gdsn(
      node = metadata.node, path = "blacklist.individuals", silent = TRUE))$R.class[1]
  ...
  if (extract) {
    bl.i.gds <- tibble::tibble(
      INDIVIDUALS = gdsfmt::read.gdsn(gdsfmt::index.gdsn(
        node = metadata.node, path = "blacklist.individuals/INDIVIDUALS", silent = TRUE)),
      FILTER = gdsfmt::read.gdsn(gdsfmt::index.gdsn(
        node = metadata.node, path = "blacklist.individuals/FILTER", silent = TRUE))
    )
  }
...
#' 
#' @param snp.per.locus 
#' Default: \code{snp.per.locus = TRUE}.
#' 
#' @param snp.position.read 
#' Default: \code{snp.position.read = TRUE}.
#' 
#' @param mac 
#' Default: \code{mac = TRUE}.
#' 
#' @param allele.coverage 
#' Default: \code{allele.coverage = TRUE}.
#' 
#' @param coverage 
#' Default: \code{coverage = TRUE}.
#' 
#' @param heterozygosity 
#' Default: \code{heterozygosity = TRUE}.
#' 
#' @param missing 
#' Default: \code{missing = TRUE}.
#' 
#' @param markers 
#' Default: \code{markers = TRUE}.
#' 
#' @param individuals 
#' Default: \code{individuals = TRUE}.
#' 
#' @export
generate_stats <- function(
  ...
  if (snp.per.locus) {
    ...
    m.stats %<>%
      dplyr::bind_rows(
        tgbase::tibble_stats(x = m.info$SNP_PER_LOCUS, group = "SNPs per locus", subsample = subsample.info)
      )
  } # snp.per.locus
...
#' 
#' @param stats.id 
#' Default: \code{stats.id = c("missing", "heterozygosity", "coverage")}.
#' 
#' @export
...
#' @keywords internal
#' @param parallel.core 
#' Default: \code{parallel.core = TRUE}.
#' 
#' @export
...
write_gds <- function(
  ...
  if (open) data.gds <- read_genome(data.gds, verbose = FALSE)
  return(data.gds)
} # End write_gds
