...
#' With the default, Will use all markers.

#' Default: \code{sample.markers = NULL}.
...
#' allele frequency.
#' With the default, Will not compute allele frequency.

#' Default: \code{allele.freq = NULL}.
...
write_colony <- function(
  ...
  tgbase::function_header(f.name = "write_colony", verbose = verbose)
  timing <- tgbase::tic()
  on.exit(tgbase::toc(timing), add = TRUE)
  on.exit(tgbase::function_header(f.name = "write_colony", start = FALSE, verbose = verbose), add = TRUE)
  ...
  if (data.type %in% c("SeqVarGDSClass", "gds.file")) {
    ...
    data <- tidy_genome(data = data, parallel.core = parallel.core)
    data.type <- "tbl_df"
  } else {
...
radiator_colony <- function(
  ...
  if (!is.null(allele.freq)) {
    ...
    freq <- input.alleles %>%
      ...
      tgbase::trans_wide(x = ., formula = "MARKERS + ALLELES_FREQ ~ GROUP", values_from = "VALUE") %>%
  ...
  data <- tidyr::unite(data = data, col = MARKERS.ALLELE_GROUP, MARKERS, ALLELE_GROUP, sep = ".") %>%
    tgbase::trans_wide(x = ., formula = "POP_ID + INDIVIDUALS ~ MARKERS.ALLELE_GROUP", values_from = "ALLELES") %>%
