...
#' \item MARKERS: stable identifier composed of a filesystem-safe chromosome
#' component, LOCUS, and POS, separated by two underscores. De novo markers use
#' \code{DENOVO__LOCUS__POS}. A reference chromosome such as
#' \code{NC_054479.1} remains unchanged in CHROM but is encoded as
#' \code{NC_054479_1} inside MARKERS. Genomic coordinates must be read from
#' CHROM and POS rather than recovered by splitting MARKERS.
#' \item CHROM: the exact reference sequence name; for de novo markers: DENOVO.
...
clean_normalize_dart_metadata <- function(x, dart.check, blacklist.id = NULL, strata = NULL) {
  ...
  dart.markers.problem %<>%
    dplyr::mutate(
      ...
      CHROM_OPTION_1 = dplyr::if_else(
        is.na(CHROM_OPTION_1), "DENOVO", CHROM_OPTION_1, missing = NA_character_
      ),
      CHROM_OPTION_2 = stringi::stri_extract_first_regex(LOCUS, "(?<=scaffold_)\\d+(?=\\|)"),
      CHROM_OPTION_2 = dplyr::if_else(
        is.na(CHROM_OPTION_2), "DENOVO", paste0("CHROM_", CHROM_OPTION_2), missing = NA_character_
      ),
      CHROM_OPTION_3 = "DENOVO",
  ...
  need.user.input <- !( !anyNA(chrom1) && length(chrom1.unique) == 1L && chrom1.unique == "DENOVO" )
