...
#' Look for \pkg{genometranslator} \code{\link{tidy_genome}}.
...
#' Default: \code{filename = "gsi_sim.unname.txt"}.
...
write_gsi_sim <- function(
  ...
  check <- genometranslator::check_pop_levels(pop.levels = pop.levels,
  ...
  if (is.null(strata)) {
    ...
    data <- genometranslator::join_strata(data = data, strata = strata, pop.id = TRUE, verbose = FALSE)
  }
