...
#'
#'
#' Used internally in \pkg{genometranslator}
...
#' \item \code{ALT_DOSAGE}: alternate-allele dosage for biallelic markers:
#' \code{0}, \code{1}, or \code{2} copies of the ALT allele, and \code{NA}
...
#' \pkg{genometranslator} \code{\link{tidy_genome}}
...
calibrate_alleles <- function(
  ...
  original.gt <- genometranslator::detect_gt(x = data, keep.one = FALSE)

  # No need to go through all the code if just GT...
  if (all(length(original.gt) == 1, original.gt == "GT")) {

    if (all(!alt.dosage, !gt.vcf, !gt.vcf.nuc)) {
      return(list(input = data, biallelic = biallelic))
    } else {
  ...
  #     data <- genometranslator::gt_recoding(x = data, gt = FALSE, gt.vcf = FALSE, arrange = FALSE) # faster...
  #     gt.vcf.nuc <- TRUE
  #   }
  ...
  if (detect.gt == "GT_VCF_NUC") {
    ...
    # to make the code as precise as ALT_DOSAGE I need to account for
  ...
  # ALT_DOSAGE
  if (detect.gt == "ALT_DOSAGE") {
    switch.ref <- data %>%
      dplyr::select(tidyselect::any_of(c("MARKERS", "ALT_DOSAGE"))) %>%
      dplyr::filter(!is.na(ALT_DOSAGE)) %>%
      dplyr::count(MARKERS, ALT_DOSAGE) %>%
      dplyr::group_by(MARKERS) %>%
      dplyr::summarise(
        REF = sum((2 * n[ALT_DOSAGE == 0L]), n[ALT_DOSAGE == 1L], na.rm = TRUE),
        ALT = sum(2 * n[ALT_DOSAGE == 2L], n[ALT_DOSAGE == 1L], na.rm = TRUE),
        .groups = "drop"
      ) %>%
  ...
  if (n.switch > 0) {
    ...
    remove.gt <- setdiff(c("GT", "ALT_DOSAGE", "GT_VCF", "GT_VCF_NUC"), detect.gt)
    ref.depth <- FALSE
    ...
    data <- dplyr::bind_rows(
      ...
      data %>%
        ...
        {if (detect.gt == "ALT_DOSAGE") dplyr::mutate(.data = .,ALT_DOSAGE = dplyr::recode(ALT_DOSAGE, '2' = 0L, '0' = 2L, .missing = NULL)) else .} %>%
