...
#' With the default, With default, \code{SAMPLE_TYPE} is filled
...
write_rubias <- function(
  ...
  if (data.type %in% c("SeqVarGDSClass", "gds.file")) {
    if (data.type == "gds.file") data %<>% genometranslator::read_genome(data = ., verbose = FALSE)
    strata.id <- genometranslator::extract_individuals_metadata(
      gds = data,
    ...
    if (rlang::has_name(data, "ALT_DOSAGE")) {
      want <- c("MARKERS", "POP_ID", "INDIVIDUALS", "ALT_DOSAGE")
      gt.format <- "ALT_DOSAGE"
    } else {
    ...
    data %<>%
      ...
      tgbase::trans_wide(x = ., formula = "POP_ID + INDIVIDUALS ~ MARKERS_ALLELES", values_from = "ALLELES")
  }
